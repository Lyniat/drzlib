...
mrb_value rz_compress_string(mrb_state *mrb, mrb_value self) {
  ...
  if (level < -1 || level > 9) {
    DL mrb_raisef(
        mrb, DL mrb_exc_get_id(mrb, DL mrb_intern_lit(mrb, "ArgumentError")),
...
mrb_value rz_decompress_string(mrb_state *mrb, mrb_value self) {
  ...
  if (size != expected_size) {
    DL mrb_raisef(mrb,
                  DL mrb_exc_get_id(mrb, DL mrb_intern_lit(mrb, "IndexError")),
...
void drb_register_c_extensions_with_api(mrb_state *mrb, struct drb_api_t *api) {
  ...
  DL mrb_define_method_id(mrb, str, DL mrb_intern_lit(mrb, "compress"),
                          rz_compress_string, MRB_ARGS_OPT(1));
  DL mrb_define_method_id(mrb, str, DL mrb_intern_lit(mrb, "decompress"),
