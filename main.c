...
mrb_value rz_compress_string(mrb_state *mrb, mrb_value self) {
  ...
  if (level < -1 || level > 9) {
		DL(mrb_raisef)(mrb, E_ARGUMENT_ERROR,
                  "expected `level` to be in the range 0..9, got %d", level);
  }
  ...
  *(size_t *)RSTR_PTR(res_rstr) = htobe64(len);
...
mrb_value rz_decompress_string(mrb_state *mrb, mrb_value self) {
  ...
  if (len < 8) {
		DL(mrb_raise)(mrb, mrb->eStandardError_class,
                 "tried to decompress a string that definitely does not have a "
                 "length tag");
  }

  size_t size = be64toh(*(size_t *)str);
  ...
  if (size != expected_size) {
		DL(mrb_raisef)(mrb, mrb_exc_get_id(mrb, mrb_intern_lit(mrb, "IndexError")),
                  "size mismatch (data corrupted) (expected %d, got %d)",
                  expected_size, size);
  }
...
void drb_register_c_extensions_with_api(mrb_state *mrb, struct drb_api_t *api) {
  ...
  DL(mrb_define_method_id)(mrb, str, mrb_intern_lit(mrb, "compress"),
                          rz_compress_string, MRB_ARGS_OPT(1));
  DL(mrb_define_method_id)(mrb, str, mrb_intern_lit(mrb, "decompress"),
                          rz_decompress_string, MRB_ARGS_NONE());
}
